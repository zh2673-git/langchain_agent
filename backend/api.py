...
from .agents.langchain_native_agent import LangChainNativeAgent
from .agents.langchain_core_agent import LangChainCoreAgent
...
class AgentAPI:
    ...
    def __init__(self):
        ...
        self.current_agent: Optional[AgentBase] = None
    ...
    async def initialize(self) -> bool:
        """初始化 API"""
        try:
            # 初始化所有 Agent（优化后的Agent类型）
            agent_classes = {
                "langchain_core": LangChainCoreAgent,     # ⭐ 推荐：LangChain核心实现
                "adaptive": AdaptiveAgent,                # 🧠 智能：自适应Agent
                "langchain_native": LangChainNativeAgent  # 🔧 原生：LangChain原生实现
            }
            
            for agent_type, agent_class in agent_classes.items():
                try:
                    # 根据Agent类型使用不同的构造参数
                    if agent_type in ["langchain_core", "langchain_native"]:
                        # LangChain实现使用默认参数
                        agent = agent_class()
                    else:
                        # 自适应Agent使用name参数
                        agent = agent_class(name=f"{agent_type}_agent")
            ...
            # 设置默认 Agent - 优先使用LangChain核心实现
            if "langchain_core" in self.agents:
                self.current_agent = self.agents["langchain_core"]
                self.current_agent_type = "langchain_core"
            elif "chain" in self.agents:
